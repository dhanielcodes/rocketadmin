...
function UpdateEmployeeModal({ closeinviteAgent, item }) {
  console.log(item?.paymentProviderSupportedCurrency);

  const [selectedCountry, setSelectedCountry] = useState();

  const [array, setArray] = useState(item?.paymentProviderSupportedCurrency);
  ...
  async function UpdateProvider() {
    mutate({
      id: item?.id,
      paymentProviderSupportedCurrency: array?.map((item) => {
        return {
          id: item?.id,
        };
      }),
  ...
  return (
    <Content>
      <Modal title="Update Employee" onClick={() => closeinviteAgent(false)}>
        <div
          className="name"
          style={{
            marginTop: "20px",
