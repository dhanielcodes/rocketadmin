...
const DroplistAdmin = ({ action, setModal, setUserId, viewDetails }) => (
  //   <Menu.Item key='1' onClick={() => onNavigate(id)}>
  <Menu
    style={{
      borderRadius: "10px",
...
function TransferLogsTable({ category, showFilter = false, typeee }) {
  ...
  const { mutate: revertpaymentMutation, isLoading: loadingrevertpayment } =
    useMutation({
      ...
      action2: (
        ...
        >
          ...
          >
            <Dropdown
              droplist={
                userDetails?.email === "admin@transferrocket.co.uk" ? (
                  <DroplistAdmin
                    action={setCall}
                    setModal={setModal}
                    viewDetails={() => {
                  ...
                  >
                    ...
                    <Btn
                      clicking={() => {
                        ...
                        } else if (call === "revertPay") {
                          revertpaymentMutation(userIdd.paymentRef);
                        } else {
